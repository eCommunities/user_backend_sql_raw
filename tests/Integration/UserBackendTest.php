...
class UserBackendTest extends TestCase {
	...
	public function testDisplayNameOfUserCanBeChanged() {
		...
		self::assertTrue($this->userBackend->setDisplayName('alice', 'Alice Alisson-Balisson'));
		self::assertSame('Alice Alisson-Balisson', $this->userBackend->getDisplayName('alice'));
	}
	...
	public function testDisplayNameCanContainNonAsciiCharacters() {
		...
		self::assertTrue($this->userBackend
			->setDisplayName('alice', 'Aliçé Alissôn-Balìsßon the 3rd van Høuten фжщй'));
		self::assertSame('Aliçé Alissôn-Balìsßon the 3rd van Høuten фжщй'
			, $this->userBackend->getDisplayName('alice'));
	}
